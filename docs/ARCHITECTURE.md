...
[OpenClaw Gateway]
    ...
    │   ├── Monitor Suite (boredom, stability, ArXiv, system health)
    │   ├── Legacy Cron Signals (optional migration review only)
    ...
    │   └── Research Analysis
    │
    └── Cron-Era Inputs (historical/reference only)
        └── Optional migration review signals
```
...
Replaces: Stability Monitor, Boredom Scan, ArXiv Monitor
- **Boredom monitor:** Continuous calculation, triggers when threshold hit
- **Stability monitor:** Memory, CPU, drive health
- **ArXiv monitor:** Real-time paper detection, auto-download
- **Legacy cron signal monitor:** Disabled by default; migration review only
...
### Phase 2: Lean Analysis Turns
- Replace GRAO analysis cron with reviewed proposal turns
- Replace GNW drive cron with plugin-fed drive computation
