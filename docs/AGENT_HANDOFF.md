...
The canonical collaboration surface is the current project GitHub repository.

The reconciled baseline is `main` at `6bbd046`. Older local Fedora and
plugin-docs copies are source material only. Neither local copy is canonical by
itself.
...
- Andi: Fedora-side orchestration, source inventory, repo coordination, and
  handoff validation.
- Plugin/docs lane: repo hygiene, architecture framing, plugin lane mapping,
...
Do not start these implementation/review lanes until the source-material
cross-exam receives owner signoff.
...
- Add fixtures only under `tests/fixtures/` with explicit owner approval.
