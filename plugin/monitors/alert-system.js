...
const SEVERITY = {
  ...
  high_memory: 'warning',
  high_cpu: 'warning',
  historical_scheduler_failure: 'error',
  historical_scheduler_stale: 'warning',
  arxiv_paper: 'info',
...
class AlertSystem extends EventEmitter {
  constructor(opts = {}) {
    super();
    this.opts = opts;
    this.alertFile = resolveDataFile(opts, 'alerts.json');
    this.broadcastFile = resolveDataFile(opts, 'broadcast-queue.json');
    this._alerts = [];
    this._queue = [];
